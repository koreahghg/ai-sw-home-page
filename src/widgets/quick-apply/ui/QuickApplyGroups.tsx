...
export default function QuickApplyGroups() {
  return (
    <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
      {GROUPS.map((group) => (
        <div key={group.key} className="flex flex-col rounded-2xl bg-white p-6">
          <div className="flex items-center gap-3">
            <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-[#F2F3F5] text-xl">{group.icon}</span>
            <div>
              <p className="text-lg font-bold text-gray-900">{group.title}</p>
              <p className="text-xs text-gray-500">{group.desc}</p>
            </div>
          </div>

          <div className="mt-5 flex flex-1 flex-col gap-2 border-t border-gray-200 pt-5">
            {group.items.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className="group flex flex-col gap-1.5 rounded-xl bg-[#F2F3F5] px-4 py-3.5 transition hover:bg-brand-light"
              >
                <div className="flex items-center justify-between gap-2">
                  ...
                  <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-white text-sm text-gray-500 transition group-hover:bg-brand group-hover:text-white">
                    →
                  </span>
                </div>
